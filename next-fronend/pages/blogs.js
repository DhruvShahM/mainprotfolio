                    ...
                    <div className="mx-auto grid w-full grid-cols-1 gap-6 pt-12 sm:w-3/4 lg:w-full lg:grid-cols-3 xl:gap-10">
                        {blogs.map((item) => {
                            ...
                            return <Link key={item.slug.current} href={"/blog" + item.slug.current} className="shadow cursor-pointer">
                                <div>
                                    <div style={{"backgroundImage": `url(${item.blogimage ? builder.image(item.blogimage).width(200).url() : '/assets/img/post-01.png'})`}}
                                        ...
                                        <span
                                            className="absolute right-0 bottom-0 mr-4 mb-4 block rounded-full bg-purple-700 border-2 border-purple-700 px-6 py-2 text-center font-body text-sm font-bold uppercase text-purple md:text-base">Read
                                            More</span>
                                    </div>
